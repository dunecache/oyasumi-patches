...
- Because the click hook's `host` argument is the `Lb2/e;` wrapper rather than an `Activity`, the extension resolves the `Activity` from the `PopupMenu` anchor view's context and caches it in a `WeakReference` at menu-show time as a fallback.
